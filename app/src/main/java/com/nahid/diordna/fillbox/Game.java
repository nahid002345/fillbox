...
public class Game implements Parcelable {
	...
	public static final char LINE = '4';
	...
	public Collection<Integer> setLine(int index){
		return setLine(index % board.getBoardSize(), index / board.getBoardSize(),index);
	}
	
	
	private Collection<Integer> setLine(int x, int y,int index){
		HashSet<Integer> changes = new HashSet<Integer>();
		
		Log.i("SetLine","X: "+x+" Y: "+y);
		try{
			checkRunning();
//    		Log.i("SetLine","runningChecked");
//			checkVacancy(x, y);
//			Log.i("SetLine","CheckVacancy");
			Board newBoard = board.clone();
//			Log.i("SetLine","Newboard");
			newBoard.setStone(x, y, Game.LINE);
			
//			Log.i("SetLine","LineStone set on board");
			
			HashMap<Character, Integer> capturesCount = changeCaptures(doCapturesLine(newBoard, x, y), history.current().getCaptures(), ADD);
//			Log.i("SetLine","HaspMap set");
//			nextTurn = invertColor(nextTurn);
			Situation s = new Situation(newBoard.toString(), Game.LINE, capturesCount);
//			Log.i("SetLine","situation set");
			
			history.add(s);
//			Log.i("SetLine","History added");
//			for(int i = 0; i < board.getPosition().length; i++){
////				if(newBoard.getPosition()[i] != board.getPosition()[i]){
////					Log.i("SetLine","enter into the loop of changes");		
//					changes.add(i);
////				}
//			}
			changes.add(index);
			
//			Log.i("SetLine","board position changed add");
...
//		}
	...
	private HashMap<Character, Integer> doCapturesLine(Board board, int x, int y) {
		HashMap<Character, Integer> capturesCount = new HashMap<Character, Integer>();
		Point stone = new Point(x, y, board.getColor(x, y));
		HashSet<Point> removenda = null;
		int count;
		for(Point p : board.getSurroundingLine(x, y)){
			if(p.getColor() == invertColor(stone.getColor()) && board.isCaptured(p.getX(), p.getY())){
				removenda = board.getChain(p.getX(), p.getY());
				if(capturesCount.containsKey(p.getColor()))
					count = capturesCount.get(p.getColor()) + removenda.size();
